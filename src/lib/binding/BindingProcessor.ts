...
export class BindingProcessor {
    ...
    public generateCodeForXMLFile(file: File, program: Program, astEditor: Editor, entry?: TranspileObj) {
    ...
    public addNodeVarsMethodForRegularXMLFile(file: File, astEditor: Editor) {
        if (!file || file.fileType !== FileType.Xml) {
            throw new Error('was given a non-xml file');
        }
    ...
    private addBindingMethodsForFile(file: File, astEditor: Editor) {
    ...
    private addFindNodeVarsMethodForFile(file: File, astEditor: Editor) {
    ...
    private addVMConstructor(file: File, astEditor: Editor) {
